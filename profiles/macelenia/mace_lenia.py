...
class MaCELenia(Lenia):
    ...
    def draw(self):
        ...
        """
        # assert self.state.shape[0] == 1, "Batch size must be 1 to draw" # (C,H,W), pygame conversion done later

        if self.C == 1:
            toshow = self.state.expand(-1,3, -1, -1)  # (3,H,W)
        elif self.C >=3:
            toshow = self.state[:,:3, :, :]  # (3,H,W)
        else:
