...
def gen_batch_params(batch_size):
    """ Generates batch parameters."""
    mu = torch.rand((batch_size,3,3))
    sigma = mu/(3*np.sqrt(2*np.log(2)))*(1+ (torch.ones_like(mu)-2*torch.rand_like(mu)))
    ...
    params = {
        ...
        'mu':  mu ,
        'sigma' : sigma,
        'beta' : torch.rand((batch_size,3,3,3)),
        'mu_k' : torch.rand((batch_size,3,3,3)),
        'sigma_k' : torch.rand((batch_size,3,3,3)),
        'weights' : torch.rand((batch_size,3,3)) # element i, j represents contribution from channel i to channel j
    }
    return params
